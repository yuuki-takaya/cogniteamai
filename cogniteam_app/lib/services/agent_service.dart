  ...
  Future<List<Agent>> getAllAgents() async {
    try {
      final response = await _apiService
          .get('/agents'); // Backend endpoint for listing agents
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        throw Exception(
            'Failed to load agents: ${response.statusMessage} ${response.data}');
      }
    } on DioException catch (e) {
      final errorMsg =
          e.response?.data?['detail'] ?? e.message ?? "Failed to load agents";
