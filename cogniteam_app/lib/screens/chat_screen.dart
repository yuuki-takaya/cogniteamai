...
import 'package:cogniteam_app/providers/auth_provider.dart'; // For current user ID & insightServiceProvider
import 'package:cogniteam_app/providers/insight_provider.dart'; // For groupInsightProvider
import 'package:cogniteam_app/models/message.dart';
import 'package:cogniteam_app/models/insight.dart'; // For Insight model
...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  // For Insights
  String _selectedInsightType = "summary"; // Default insight type
  final List<String> _insightTypes = ["summary", "sentiment", "keywords"];
  AsyncValue<Insight>? _currentInsightValue; // To hold the state of the insight fetch

  @override
  void initState() {
    super.initState();
    // Optionally, trigger initial actions if not handled by provider's init
    // Fetch initial summary insight when screen loads
    _fetchInsight(_selectedInsightType);
  }
  ...
  Future<void> _fetchInsight(String insightType) async {
    // Manually trigger the FutureProvider and store its state.
    // This allows us to show loading/error specifically for the insight section.
    setState(() {
      _currentInsightValue = const AsyncValue.loading(); // Show loading for insight
    });
    try {
      final insight = await ref.read(groupInsightProvider(
        InsightProviderParams(groupId: widget.groupId, insightType: insightType)
      ).future);
      setState(() {
        _currentInsightValue = AsyncValue.data(insight);
      });
    } catch (e, s) {
      setState(() {
        _currentInsightValue = AsyncValue.error(e,s);
      });
    }
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          _buildInsightSection(), // Add insight section here
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  const Text("Conversation Insights", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  DropdownButton<String>(
                    value: _selectedInsightType,
                    items: _insightTypes.map((String type) {
                    ...
                    onChanged: (String? newValue) {
                      if (newValue != null) {
                        setState(() {
                          _selectedInsightType = newValue;
                        });
                        _fetchInsight(newValue); // Fetch new insight when type changes
                      }
                    },
                  ),
                ],
              ...
              _currentInsightValue == null
                ? const Text("Select an insight type to view.")
                : _currentInsightValue!.when(
                    loading: () => const Center(child: CircularProgressIndicator(strokeWidth: 2.0)),
                    error: (err, stack) => Column(
                      children: [
                        Text("Error loading insight: $err", style: TextStyle(color: Theme.of(context).colorScheme.error)),
                        TextButton(onPressed: () => _fetchInsight(_selectedInsightType), child: const Text("Retry"))
                      ],
                    ),
                    data: (insight) => Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(insight.insightText, style: const TextStyle(fontSize: 13)),
                        const SizedBox(height: 4),
                        Text(
                          "Type: ${insight.insightType.capitalize()} | Generated: ${insight.generatedAt.toLocal().toString().substring(0,16)}",
